...
Я удалил compose.yml было предупреждение, о том что нужно переименовать так новая версия не использует docker-compose. Так же при запуске был WARN, который говорит нам о том, что в файле манифесте не нужно указать версию composeю
